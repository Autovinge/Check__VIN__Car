  ...
  try {
    ...
    if (!res.ok) {
      ...
      try {
        ...
        throw new Error(`BOG API error: ${res.status} - ${JSON.stringify(errorJson)}`)
      } catch {
    ...
    const transactionId =
      response.orderRequestId || response.transactionId || response.id || response.external_order_id;
    let transactionUrl =
      (response._links && response._links.redirect && response._links.redirect.href) ||
      response.redirectUrl || response.transactionUrl || response.url;
